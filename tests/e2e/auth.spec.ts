...
test('real login, profile edit, public profile and logout',async({page})=>{
  ...
  try {
    ...
    await expect(page.locator('#reward-summary')).toContainText('1 枚臭鸡蛋');
    await expectLive(page.locator('#ledger-list')).toContainText('每日签到');
    const badge=page.locator('.task-card').filter({has:page.getByRole('heading',{name:'初次赴约',exact:true})});
    await badge.getByRole('button',{name:'展示这枚勋章'}).click();
    await expect(badge.getByRole('button',{name:'正在展示 · 取消'})).toBeVisible();
    await page.goto('/notifications/');
    await expect(page.locator('#notification-list')).toContainText('初次赴约');
    await page.getByRole('button',{name:'全部标为已读'}).click();
    await expect(page.locator('.notification-card[data-unread="true"]')).toHaveCount(0);
    ...
    await expect(page.locator('.badge-wall')).toContainText('★ 初次赴约');
    await page.locator('.user-menu summary').click();
