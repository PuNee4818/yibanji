...
if(list){
 const tab=new URLSearchParams(location.search).get('tab')??'hot';let page=0;const more=document.querySelector<HTMLButtonElement>('[data-more-feed]');
 ...
 async function refresh(append=false){
  ...
  try{const rows=await rpc<Discussion[]>('community_feed',{p_tab:tab,p_page:page});if(!append)list!.replaceChildren();if(!rows.length&&!append){const p=document.createElement('p');p.className='empty-state';p.textContent=tab==='following'?'还没有关注的书友动态。去最新讨论里，认识正在交流的人。':'这里还安静得像一本刚翻开的书。留下第一段感想吧。';list!.append(p);}for(const row of rows)list!.append(discussionCard(row,user?.id,()=>refresh()));if(more)more.hidden=rows.length<20;}catch(error){list!.textContent=(error as Error).message;}
 }
 more?.addEventListener('click',()=>{page++;void refresh(true);});await refresh();
