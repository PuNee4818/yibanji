...
async function initializeCommunity() {
  ...
  if (highlights) {
    try {
      const [data, { data: discussed, error }] = await Promise.all([
        rpc<{
      ...
      for (const [label, rows] of [
        ...
        if (!rows.length) {
          const p = document.createElement('p');
          p.className = 'muted';
          p.textContent = '本周暂无排行，去目录挑一篇读读吧。';
          highlights.append(p);
        }
        for (const row of rows) {
          const p = document.createElement('p');
    ...
    const render = async () => {
      ...
      if (data) {
        ...
        target.replaceChildren(
          discussionCard(data as Discussion, user?.id, render, undefined, true),
        );
      }
    };
    await render();
...
// This module is also used on the homepage. A slow discussion request must not
// hold up its sibling modules (continue reading, bookmarks, and navigation).
void initializeCommunity().catch(() => {
  document.querySelectorAll('[data-feed-description],[data-recent-discussions]').forEach((node) => {
    node.textContent = '讨论暂时无法加载，请刷新重试。';
  });
});
