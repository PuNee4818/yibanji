...
if(section){
 const kind=section.dataset.discussionKind!;const target=section.dataset.discussionTarget!;const {data:{user}}=await supabase.auth.getUser();
 ...
 async function refresh(append=false){
  try{const rows=await rpc<Discussion[]>('discussion_thread',{p_kind:kind,p_target:target,p_sort:sort.value,p_page:page});if(!append)list.replaceChildren();
   ...
   const reply=(item:Discussion)=>{parent=item.parent_id??item.id;hint.textContent=`回复 ${item.display_name}`;hint.hidden=false;cancel.hidden=false;form.scrollIntoView({block:'center'});form.querySelector('textarea')?.focus();};
 ...
 form.addEventListener('submit',async event=>{event.preventDefault();if(!user){location.href='/auth/';return;}const body=String(new FormData(form).get('content'));const fingerprint=JSON.stringify([body,parent]);if(submitted!==fingerprint){key=crypto.randomUUID();submitted=fingerprint;}const button=form.querySelector('button')!;button.disabled=true;try{await rpc('save_discussion',{p_kind:kind,p_target:target,p_content:body,p_parent:parent,p_key:key});form.reset();resetReply();submitted=undefined;page=0;await refresh();announce('这段话已经留在这里了。');}catch(error){announce((error as Error).message);}finally{button.disabled=false;}});
 await refresh();
}
