...
- 阶段 5：growth/economy migration 已应用；全套 build/typecheck/lint/unit/content/16 项浏览器测试通过，新增真实签到 UI 验收；两组真实数据库集成测试通过，涵盖并发签到、断签、七档循环、30 天成就与一次性奖励、日周任务、账本对账和私有数据隔离。
- 阶段 6：文章目录、互动、阅读会话迁移已应用。全部常规门禁与 16 项浏览器测试通过；真实余额 5 并发投 5/5 仅一次成功、余额 0、统计 +5；验证多枚/重复投掷、幂等参数冲突、点赞并存、旧收藏事务导入、有效阅读时间门槛和私有进度隔离。
